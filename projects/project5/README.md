...
Due: July 20th at 11:59pm (Late: July 21st at 11:59pm)
...
* Note: You must submit all the files in `src` 
  (i.e. `src/basics.rs src/communicator.rs src/lib.rs src/locator.rs`) even if you edited only one of them.

Compilation and Tests
---------------------
In order to compile the project, simply run `cargo build`. To test, run `cargo test` in the root directory of the project. 
The __tests won't run if any part of the project does not compile__.
...
Part 4: Communicator (Extra Credit)
--------------------
